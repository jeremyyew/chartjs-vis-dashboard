...
def getReviewInfo(request):
    ...
    if request.body:
        ...
        for submissionID in submissionIDs:
            reviews = [str(line[evaluationIndex]).replace("\r", "") for line in reviewData if
                       str(line[submissionIDIndex]) == submissionID]
            # print reviews
...
def getSubmissionInfo(request):
    ...
    if request.body:
        ...
        paperGroupsByTrack = {track: [line for line in submissionData if str(line[trackNameIndex]) == track] for track
                              in tracks}
        ...
        for track, papers in paperGroupsByTrack.items():
            ...
            acceptedAuthorsThisTrack = [str(ele[authorIndex]).replace(" and ", ", ").split(", ") for ele in
