...
def getAuthorInfo(request):
	...
	if request.body:
		...
		firstNameCol = data['firstNameCol']
		lastNameCol = data['lastNameCol']
		countryCol = data['countryCol']
		affiliationCol = data['affiliationCol']

		authorList = []
		for authorInfo in authorData:
			authorList.append({'name': authorInfo[firstNameCol] + " " + authorInfo[lastNameCol],
				'country': authorInfo[countryCol],
				'affiliation': authorInfo[affiliationCol]})
